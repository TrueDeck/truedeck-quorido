    ...
    describe("tests/coverage", function() {
      ...
      describe("should behave like Withdrawer role", function() {
        beforeEach("check preconditions", async function() {
          ...
          ;(await this.contract["isWithdrawer"](otherWithdrawer)).should.equal(
            true
          )
        ...
        it("reverts when querying roles for the null account", async function() {
          await expectRevert.unspecified(
            this.contract["isWithdrawer"](ZERO_ADDRESS)
          )
        })
        ...
        describe("access control", function() {
          ...
          context("from unwithdrawer game contract", function() {
            ...
            it("reverts", async function() {
              await expectRevert.unspecified(
                this.anyGameContract["onlyWithdrawerMock"]({ from })
              )
            })
          })
        })
        ...
        describe("add", function() {
          ...
          context("from the manager account", function() {
            ...
            it("emits a WithdrawerAdded event", async function() {
              const { logs } = await this.contract["addWithdrawer"](anygame, {
                from,
              })
        ...
        describe("remove", function() {
          ...
          context("from the manager account", function() {
            it("removes role from an already assigned game contract", async function() {
              ...
              ;(await this.contract["isWithdrawer"](withdrawer)).should.equal(
                false
              )
              ;(await this.contract["isWithdrawer"](
                otherWithdrawer
              )).should.equal(true)
            })

            it("emits a WithdrawerRemoved event", async function() {
              const { logs } = await this.contract["removeWithdrawer"](
                withdrawer,
                { from }
              )
              expectEvent.inLogs(logs, "WithdrawerRemoved", {
                account: withdrawer,
              })
            })
            ...
            it("reverts when removing from an unassigned game contract", async function() {
              await expectRevert.unspecified(
                this.contract["removeWithdrawer"](anygame),
                { from }
              )
            })
            ...
            it("reverts when removing from an unassigned non-game contract", async function() {
              await expectRevert.unspecified(
                this.contract["removeWithdrawer"](anyone),
                { from }
              )
            })
            ...
            it("reverts when removing role from the null account", async function() {
              await expectRevert.unspecified(
                this.contract["removeWithdrawer"](ZERO_ADDRESS),
                { from }
              )
            })
          })
        ...
        describe("renouncing roles", function() {
          ...
          it("emits a WithdrawerRemoved event", async function() {
            const receipt = await this.gameContract["renounceWithdrawer"]({
              from: anyone,
            })
          ...
          it("reverts when renouncing unassigned role by game contract", async function() {
            await expectRevert.unspecified(
              this.anyGameContract["renounceWithdrawer"]({ from: anyone })
            )
          })

          it("reverts when renouncing unassigned role by non-game account", async function() {
            await expectRevert.unspecified(
              this.contract["renounceWithdrawer"]({ from: anyone })
            )
          })
        })
      })
