...
contract("Dice", function([
  ...
  before(async function() {
    if (mode === "profile") {
      ...
      await this.chip.transfer(this.bankroll.address, initialBankroll, {
        from: initialHolder,
      })
      ;(await this.chip.balanceOf(
        this.bankroll.address
      )).should.be.bignumber.equal(initialBankroll)

      // Player has enough balance
      await this.chip.transfer(player, initialBalance, { from: initialHolder })
      ;(await this.chip.balanceOf(player)).should.be.bignumber.equal(
        initialBalance
      )

      // Player approves an amount
      await this.chip.approve(this.bankroll.address, approveAmount, {
        from: player,
      })
    }
  })
    ...
    describe("tests/coverage", function() {
      ...
      context("initialized", async function() {
        ...
        it("dice contract has the withdrawer role", async function() {
          ;(await this.bankroll.isWithdrawer(this.dice.address)).should.equal(
            true
          )
        })
        ...
        describe("deposit", function() {
          describe("when the bankroll does not have enough approved balance", function() {
            describe("when the player does not have enough balance", function() {
              it("reverts", async function() {
                await expectRevert.unspecified(
                  this.dice.deposit(this.chip.address, depositAmount, {
                    from: player,
                  })
                )
              })
            ...
            describe("when the player has enough balance", function() {
              beforeEach(async function() {
                await this.chip.transfer(player, initialBalance, {
                  from: initialHolder,
                })
                ;(await this.chip.balanceOf(player)).should.be.bignumber.equal(
                  initialBalance
            ...
            describe("when the player has enough balance", function() {
              ...
              context("when unpaused", function() {
                ...
                it("deposits the requested amount", async function() {
                  await this.dice.deposit(this.chip.address, depositAmount, {
                    from: player,
                  })
                  ;(await this.dice.balanceOf(
                    player
                  )).should.be.bignumber.equal(depositAmount)
                  ;(await this.chip.balanceOf(
                    this.bankroll.address
                  )).should.be.bignumber.equal(depositAmount)
                  ;(await this.chip.balanceOf(
                ...
                it("decreases the bankroll allowance", async function() {
                  ...
                  ;(await this.chip.allowance(
                    player,
                    this.bankroll.address
                  )).should.be.bignumber.equal(approveAmount.sub(depositAmount))
                })

                it("emits a transfer event", async function() {
                  const receipt = await this.dice.deposit(
                    this.chip.address,
                    depositAmount,
                    { from: player }
                  )
              ...
              describe("game tests", function() {
                beforeEach(async function() {
                  ...
                  ;(await this.chip.balanceOf(
                    ...
                    initialBankroll.add(depositAmount)
                  )
                  ;(await this.chip.balanceOf(
                    player
                  )).should.be.bignumber.equal(
                ...
                describe("a simple game", function() {
                  ...
                  beforeEach(async function() {
                    ...
                    gameData = await DiceGameDataEncoder.create()
                      ...
                      .rollUnder("10")
                      .clientSeed(
                        "df0ea096b54fc7ef48f679c094fe2f3ff2af2dd75a228fe719e9868004a11f15"
                      )
                      .serverSeed(
                        "75f82f273177f1760120a0fb29e5572d031723dd955af840438fc7ea44d6e994"
                      )
                      .hasWon(true)
                      .___encode(signer)
                  })
                  ...
                  it("withdraws the requested amount", async function() {
                    ...
                    // Assertions
                    const expectedDiceBalanceOfPlayer = depositAmount
                      .sub(gameData.betAmount)
                      .add(gameData.payout)
                      .sub(withdrawAmount)
                    ;(await this.dice.balanceOf(
                      player
                    )).should.be.bignumber.equal(expectedDiceBalanceOfPlayer)

                    const expectedBankrollBalance = initialBankroll
                      .add(depositAmount)
                      .sub(withdrawAmount)
                    ;(await this.chip.balanceOf(
                      this.bankroll.address
                    )).should.be.bignumber.equal(expectedBankrollBalance)

                    const expectedPlayerBalance = initialBalance
                      .sub(depositAmount)
                      .add(withdrawAmount)
                    ;(await this.chip.balanceOf(
                      player
                    )).should.be.bignumber.equal(expectedPlayerBalance)
                  })

                  it("emits a transfer event", async function() {
                    // Withdraw Transaction
                    const receipt = await this.dice.withdraw(
                      this.chip.address,
                      withdrawAmount,
                      gameData.data,
                      gameData.proof,
                  ...
                  it("emits a proved event", async function() {
                    // Withdraw Transaction
                    const { logs } = await this.dice.withdraw(
                      this.chip.address,
                      withdrawAmount,
                      gameData.data,
                      gameData.proof,
                ...
                describe("a complex game", function() {
                  ...
                  beforeEach(async function() {
                    gameData = await DiceGameDataEncoder.create()
                      ...
                      .rollUnder("45")
                      .clientSeed(
                        "8c93ad444e8b965e36e2cacef852f56fe8158b1ab923f79517d036840e109ac1"
                      )
                      .serverSeed(
                        "9e570c1c6f9ce2b22c511418b436895e10a238c6e19c0a35311ab9eeb1d8371f"
                      )
                      .hasWon(false)
                      .___anotherRound()
                      .betAmount("125")
                      .rollUnder("5")
                      .clientSeed(
                        "dec6e001631613b638fe8a774fcb11cbf5f97c88ec4dea14f1584f12df15be4c"
                      )
                      .serverSeed(
                        "f54baee274c71462393187c27142e6f613ce2dadf8d7674dd1f2a39ace22015c"
                      )
                      .hasWon(true)
                      .___encode(signer)
                  })
