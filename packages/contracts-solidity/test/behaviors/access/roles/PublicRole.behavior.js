...
function shouldBehaveLikePublicRole(
  ...
  describe("should behave like public role", function() {
    ...
    describe("add", function() {
      ...
      context(
        `from ${manager ? "the manager" : "a role-haver"} account`,
        function() {
          ...
          it("reverts when adding role to an already assigned account", async function() {
            await expectRevert.unspecified(
              this.contract[`add${rolename}`](authorized, { from })
            )
          })
    ...
    describe("remove", function() {
      ...
      context(`from ${manager ? "the manager" : "any"} account`, function() {
        it("removes role from an already assigned account", async function() {
          ...
          ;(await this.contract[`is${rolename}`](authorized)).should.equal(
            false
          )
          ;(await this.contract[`is${rolename}`](otherAuthorized)).should.equal(
            true
        ...
        it(`emits a ${rolename}Removed event`, async function() {
          ...
          expectEvent.inLogs(logs, `${rolename}Removed`, {
            account: authorized,
          })
        })
        ...
        it("reverts when removing from an unassigned account", async function() {
          await expectRevert.unspecified(
            this.contract[`remove${rolename}`](anyone),
            { from }
          )
        })
    ...
    describe("renouncing roles", function() {
      ...
      it(`emits a ${rolename}Removed event`, async function() {
        const { logs } = await this.contract[`renounce${rolename}`]({
          from: authorized,
        })
      ...
      it("reverts when renouncing unassigned role", async function() {
        await expectRevert.unspecified(
          this.contract[`renounce${rolename}`]({ from: anyone })
        )
      })
    })
  })
