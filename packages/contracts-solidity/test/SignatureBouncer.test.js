    ...
    describe("tests/coverage", function() {
      ...
      context("signature validation", function() {
        it("invalidates wrong hash", async function() {
          ...
          const invalid_hash =
            "0xabcd48380eecc4832905500b23878950f5c00bf985086a4552d31285fcaf4519"
          ;(await this.sigBouncer.checkValidDataHash(
            h1,
            await signMessage(signer, invalid_hash)
          )).should.equal(false)
        })
        ...
        it("invalidates hash by unauthorized signer", async function() {
          ...
          ;(await this.sigBouncer.checkValidDataHash(
            h1,
            await signMessage(otherSigner, h1)
          )).should.equal(false)
        })
