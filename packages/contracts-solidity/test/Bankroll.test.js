      ...
      context("initialized", async function() {
            ...
            describe("when the player does not have enough balance", function() {
              it("reverts", async function() {
                await expectRevert.unspecified(
                  this.gameContract.deposit(token, depositAmount, {
                    from: player,
                  })
                )
              })
            ...
            describe("when the player has enough balance", function() {
              ...
              context("when unpaused", function() {
                ...
                it("deposits the requested amount", async function() {
                  await this.gameContract.deposit(token, depositAmount, {
                    from: player,
                  })
                  ;(await this.gameContract.balanceOf(
                    player
                ...
                it("emits a transfer event", async function() {
                  const receipt = await this.gameContract.deposit(
                    token,
                    depositAmount,
                    { from: player }
                  )
        ...
        describe("withdrawal", function() {
          ...
          describe("when the bankroll has enough balance", function() {
            beforeEach(async function() {
              await this.chip.transfer(spender, initialBankroll, {
                from: initialHolder,
              })
              ;(await this.chip.balanceOf(spender)).should.be.bignumber.equal(
                initialBankroll
              )
            })
            ...
            context("when unpaused", function() {
              ...
              it("withdraws the requested amount", async function() {
                ...
                ;(await this.gameContract.balanceOf(
                  player
                )).should.be.bignumber.equal(zeroAmount)
                ;(await this.chip.balanceOf(player)).should.be.bignumber.equal(
                  withdrawAmount
                )
                ;(await this.chip.balanceOf(spender)).should.be.bignumber.equal(
                  initialBankroll.sub(withdrawAmount)
                )
              })
